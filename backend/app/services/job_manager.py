...
class GenerateBundleProvider(Protocol):
    def __call__(
        self,
        *,
        job_id: str,
...
class UsageReporter(Protocol):
    def __call__(self, *, job_id: str, owner_user_id: str, attempt: int, job_dir: Path) -> None: ...
...
class JobManager:
    ...
    def claim_next_queued_job(self, *, machine_id: str) -> dict[str, str] | None:
        """Claim one queued job for an independent judge machine.

        Args:
            machine_id: Logical id of the current judge worker.
        ...
        for _created_at, job_id in sorted(candidates):
            payload = self._try_claim_job(job_id=job_id, machine_id=machine_id)
    ...
    def run_claimed_job(self, *, job_id: str, owner_user_id: str) -> None:
        ...
        Args:
            ...
            owner_user_id: Job owner identifier.
        """

        self._run_job_thread(job_id=job_id, owner_user_id=owner_user_id)

    def release_judge_claim(self, *, job_id: str, claim_id: str) -> bool:
        """Release a judge claim lock for job.

        This is used by backend MCP tool `judge.release_claim`, so the
        independent judge worker does not need to touch lock files directly.

        Args:
            job_id: Job identifier.
            claim_id: Claim token returned by `claim_next_queued_job`.

        Returns:
            True when lock is released (or already missing), False when lock
            exists but claim_id mismatches.
        """
    ...
    def cancel_job(self, *, job_id: str) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._runner_executor == "docker":
                ...
                if not stage_procs:
                    self._stop_local_pid_from_state(state=state)
    ...
    def _try_claim_job(self, *, job_id: str, machine_id: str) -> dict[str, str] | None:
        paths = get_job_paths(jobs_root=self._jobs_root, job_id=job_id)
        if not paths.state_json.exists():
            return None

        lock_path = paths.logs_dir / "judge.lock"
        self._try_break_stale_lock(lock_path=lock_path)
        claim_id = secrets.token_hex(16)
        if not self._acquire_lock(lock_path=lock_path, machine_id=machine_id, claim_id=claim_id):
            return None
    ...
    def _acquire_lock(self, *, lock_path: Path, machine_id: str, claim_id: str) -> bool:
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as fp:
            fp.write(json.dumps({"machine_id": machine_id, "claim_id": claim_id, "claimed_at": now_iso()}, ensure_ascii=False))
            fp.write("\n")
        return True

    def _try_break_stale_lock(self, *, lock_path: Path) -> None:
        if not lock_path.exists():
            return
        try:
            ...
            return
        if age >= float(self._judge_lock_stale_seconds):
            lock_path.unlink(missing_ok=True)

    def _stop_local_pid_from_state(self, *, state: dict[str, Any]) -> None:
        containers = state.get("containers") or {}
        for stage in ("generate", "test"):
            cinfo = containers.get(stage) or {}
            raw_id = str(cinfo.get("id") or "")
            if not raw_id.startswith(f"local-{stage}-pid-"):
                continue
            pid_str = raw_id.removeprefix(f"local-{stage}-pid-")
    ...
    def _parse_openai_api_key(self, *, auth_json_bytes: bytes) -> str:
        try:
    ...
    def _prepare_generate_bundle_from_db(self, *, owner_user_id: str, state: dict[str, Any]) -> GenerateBundle:
        target = None
    ...
    def _run_generate(self, *, paths: JobPaths, owner_user_id: str, attempt: int, prompt_mode: str) -> None:
        ...
        if self._generate_bundle_provider is not None:
            bundle = self._generate_bundle_provider(
            ...
            bundle = self._prepare_generate_bundle_from_db(owner_user_id=owner_user_id, state=cast(dict[str, Any], state))

        upstream_base_url = str(bundle.openai_base_url or "")
        auth_bytes = bundle.auth_json_bytes
        secret = self._parse_openai_api_key(auth_json_bytes=auth_bytes)
        ...
        if self._runner_executor == "docker":
            ...
            put_files(
                container,
                dest_dir="/codex_home",
                files={"config.toml": effective_config_toml.encode("utf-8"), "auth.json": auth_bytes},
            )
            ...
            (codex_home / "config.toml").write_text(effective_config_toml, encoding="utf-8")
