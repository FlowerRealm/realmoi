...
class JobManager:
    ...
    def run_claimed_job(self, *, job_id: str, owner_user_id: str, lock_path: str) -> None:
        """Run claimed job inline and always release claim lock.

        Args:
            job_id: Job identifier.
            owner_user_id: Job owner identifier.
            lock_path: Lock file path generated by claim step.
        """
    ...
    def _try_claim_job(self, *, job_id: str, machine_id: str) -> dict[str, str] | None:
        ...
        if not self._acquire_lock(lock_path=lock_path, machine_id=machine_id):
            return None
    ...
    def _acquire_lock(self, *, lock_path: Path, machine_id: str) -> bool:
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as fp:
            fp.write(json.dumps({"machine_id": machine_id, "claimed_at": now_iso()}, ensure_ascii=False))
            fp.write("\n")
        return True
    ...
    def _resolve_judge_api_base_url(self) -> str:
        configured = str(SETTINGS.judge_api_base_url or "").strip()
        if configured:
    ...
    def _load_self_test_token(self, *, paths: JobPaths) -> str:
        try:
            job_obj = json.loads(paths.job_json.read_text(encoding="utf-8"))
        except Exception:
            return ""
    ...
    def _run_generate(self, *, paths: JobPaths, owner_user_id: str, attempt: int, prompt_mode: str) -> None:
        ...
        self_test_token = self._load_self_test_token(paths=paths)
        self_test_url = f"{self._resolve_judge_api_base_url()}/jobs/{paths.root.name}/self-test"
        extra_env = {
            "ATTEMPT": str(attempt),
            "PROMPT_MODE": prompt_mode,
            "OPENAI_BASE_URL": upstream_base_url,
            "REALMOI_CODEX_TRANSPORT": str(SETTINGS.runner_codex_transport or "appserver"),
            "REALMOI_JUDGE_SELF_TEST_URL": self_test_url,
            "REALMOI_JUDGE_SELF_TEST_TOKEN": self_test_token,
        }
        if SETTINGS.mock_mode:
