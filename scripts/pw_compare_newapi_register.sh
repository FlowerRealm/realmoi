...
pwc_wait_url "${realmoi_register_url}" realmoi
pwc_wait_url "${newapi_register_url}" 'new-api'

echo '[3/4] playwright screenshots...'
export CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"
export PWCLI="${PWCLI:-${CODEX_HOME}/skills/playwright/scripts/playwright_cli.sh}"

template="${repo_root}/scripts/pw_compare_newapi/templates/register.pw.js.tmpl"

pwc_require_executable "${PWCLI}" PWCLI
pwc_require_file "${template}" 'template'
...
"${PWCLI}" --session "${realmoi_session}" run-code "$(
  pwc_render_template     "${template}"     "__LABEL__=realmoi"     "__URL__=${realmoi_register_url}"
)" | tee pw_realmoi.log
...
"${PWCLI}" --session "${newapi_session}" run-code "$(
  pwc_render_template     "${template}"     "__LABEL__=newapi"     "__URL__=${newapi_register_url}"
)" | tee pw_newapi.log
popd >/dev/null
...
pwc_require_file "${repo_root}/scripts/pw_compare_newapi/pairs/register.json" 'diff pairs'
python3 -X utf8 "${repo_root}/scripts/pw_compare_newapi/diff_images.py"   --out-dir "${out_dir}"   --pairs "${repo_root}/scripts/pw_compare_newapi/pairs/register.json"
