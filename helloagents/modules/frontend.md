...
- 价格字段输入改为受控输入（`onChange` 即更新状态），避免失焦与点击保存的时序导致旧值提交
- 启用模型前增加前端校验：必须填齐 4 个价格字段（in/cached_in/out/cached_out）
- 新增“实时模型来源”区块：按所选渠道实时拉取 `/admin/upstream/models` 的 `model id`
- Pricing 表格默认优先显示实时返回的模型 ID；历史残留模型不再固定占据主视图
- “实时模型来源”默认值改为“全部已启用渠道”，无需手动先选单一渠道
- 已移除来源选择字段：Pricing 页面默认自动聚合全部启用渠道并实时拉取模型
- 聚合拉取改为“容错模式”：单个渠道（如 default）401 不再阻断整体模型展示，只提示部分渠道失败
- 模型前缀展示中，空渠道不再显示 `default`，改为 `未分配` / `unassigned`
